...
function validate_email($app, $email)
{
    ...
    try {
        if (isset($mailgun['pubkey'])) {
            ...
            $external_valid = $mgClient->get('address/validate', ['address' => $email]);
...
{
    $find = R::findOne('squadriglia', ' codicecensimento = ?', [$codicecensimento]);
    if (null != $find) {
        $cc = new stdClass();
...
{
    $find = R::findOne('asa_anagrafica_eg', ' codicesocio = ?', [$codicecensimento]);
    if ($find != null) {
        $cc = new \stdClass();
        $cc->nome = $find['nome'];
        ...
        $find = R::findOne('registration', ' codicecensimento = ?', [$codicecensimento]);
        if (null != $find) {
            $cc->email = $find['email'];
...
function findDatiCapoReparto($regione, $gruppo, $legame = null)
{
    ...
    if (null != $legame) {
        ...
        if (null != $legameBean) {
            ...
            $findMyCCs = R::findAll('registration', 'regione = ? and gruppo = ? and type = ? and email = ? ', [$regione, $gruppo, 'CC', $emailCapoReparto]);
            if (null != $findMyCCs) {
                $i = 0;
        ...
        $findCCs = R::findAll('asa_capireparto_ruolo', 'creg = ? and ord = ? ', [$regione, $gruppo]);

        if ($findCCs != null) {
            $i = 0;
            foreach ($findCCs as $findCC) {
                $cc_codcens = $findCC['codicesocio'];
                $cc_anagrafica = R::findOne('asa_anagrafica_capireparto', 'codicesocio = ?', [$cc_codcens]);
                $cc = new stdClass();
                $cc->nome = $cc_anagrafica['nome'];
                $cc->cognome = $cc_anagrafica['cognome'];
                $cc->codicecensimento = $cc_codcens;

                $findccEmail = R::findOne('asa_capireparto_email', ' codicesocio = ? and tipo = ?', [$cc_codcens, 'E']);
                if (null != $findccEmail) {
