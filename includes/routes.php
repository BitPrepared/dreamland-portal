...
$app->get('/home', 'authenticate', function () use ($app) {
    ...
    if (isset($wordpress['user_info']['codicecensimento'])) {
        ...
        $ruolo = 'undefined';
        $findEG = R::findOne('asa_anagrafica_eg', ' codicesocio = ?', [$codicecensimento]);
        if (null != $findEG) {
            $ruolo = 'eg';
        } else {
            $findCC = R::findOne('asa_anagrafica_capireparto', ' codicesocio = ?', [$codicecensimento]);
            if (null != $findCC) {
                $ruolo = 'cc';
            }
        }
...
$app->get('/ordini', 'authenticate', function () use ($app) {
    ...
    foreach ($elenco as $eA) {
        ...
        if (isset($livelloAssoc[$livello]) && count($livelloAssoc[$livello]) > 10) {
            continue;
        }
    ...
    foreach ($livelloAssoc as $livello => $gruppo) {
        $dati['livelloAssocA'][$livello] = [];
        $dati['livelloAssocB'][$livello] = [];
        $dati['livelloAssocC'][$livello] = [];
