    ...
    $app->group('/sfide', function () use ($app) {
        ...
        $app->get('/:id', function ($sfida_id) use ($app) {
            ...
            try {
                ...
                $drm_iscrizione_sfida = R::findOne('iscrizionesfida', 'codicecensimento = ? and idsfida = ?', [$codicecensimento, $sfida_id]);
        ...
        $app->get('/iscrizione/:id', function ($idsfida) use ($app) {
            ...
            try {
                ...
                $squadriglia = R::findOne('squadriglia', 'codicecensimento = ?', [$codicecensimento]);
                if (null == $squadriglia) {
                ...
                $drm_iscrizione_sfida = R::findOne('iscrizionesfida', 'codicecensimento = ? and idsfida = ?', [$codicecensimento, $sfide['sfida_id']]);
            ...
            if ($app->response->getStatus() == 201) {
                $app->redirect('/portal/home#/sfide/iscr?id='.$idsfida);
            }
        ...
        $app->put('/iscrizione/:id', function ($sfida_id) use ($app) {
            ...
            try {
                ...
                try {
                    ...
                    throw new Exception('Capo del ragazzo '.$codicecensimento.' non trovato -> '.$e->getMessage(), Errori::CODICE_CENSIMENTO_NOT_FOUND);
                }

                $capoReparto = $capoRepartoArray[0];

                $to = [$capoReparto->email => $capoReparto->nome.' '.strtoupper($capoReparto->cognome[0]).'.'];
                ...
                EventManager::addEvent($codicecensimento, EventType::SFIDE, new EventElement(Sfide::ISCRITTO, []));
        ...
        $app->delete('/iscrizione/:id', function ($sfida_id) use ($app) {
            ...
            try {
                ...
                $drm_iscrizione_sfida = R::findOne('iscrizionesfida', 'codicecensimento = ? and idsfida = ? and attiva = ?', [$codicecensimento, $sfida_id, true]);
                if (null != $drm_iscrizione_sfida) {
...
//            provasuperata : true
            ...
            try {
                ...
                $drm_iscrizione_sfida = R::findOne('iscrizionesfida', 'codicecensimento = ? and idsfida = ?', [$codicecensimento, $sfida_id]);
                if (null != $drm_iscrizione_sfida) {
                    $drm_chiusura_sfida = R::findOne('chiusurasfida', 'codicecensimento = ? and idsfida = ?', [$codicecensimento, $sfida_id]);
                    ...
                    if ($q >= 0) {
                        $app->log->info('Aggiornata squadriglia rappresentata da '.$codicecensimento);
                    }
        ...
        $app->put('/conferma/:id/:cc', function ($sfida_id, $codicecensimentoeg) use ($app) {
            ...
            try {
                ...
                $drm_chiusura_sfida = R::findOne('chiusurasfida', 'codicecensimento = ? and idsfida = ?', [$codicecensimentoeg, $sfida_id]);
                if (null != $drm_chiusura_sfida) {
