    ...
    $app->group('/registrazione', function () use ($app) {
        ...
        $app->get('/info/:token', function ($token) use ($app) {
            ...
            try {
                ...
                $findToken = R::findOne('registration', ' token = ?', [$token]);
                if (null == $findToken) {
                ...
                $findGruppo = R::findOne('asa_gruppi', 'ord = ?', [$gruppo]);
                $info->gruppoNome = trim($findGruppo['nome']);

                $regione = $findToken['regione'];
                $findRegione = R::findOne('asa_regioni', 'cregione = ?', [$regione]);
                $info->regioneNome = trim($findRegione['nome']);

                $zona = $findToken['zona'];
                $findZona = R::findOne('asa_zone', 'czona = ? and cregione = ?', [$zona, $regione]);
                ...
                if ($e->getCode() == Errori::FORMATO_MAIL_NON_VALIDO) {
                    $testo = $e->getMessage();
                }
        ...
        $app->post('/step1', function () use ($app) {
            ...
            try {
                ...
                if (strcmp($codicecensimento, 'SANMARINO') == 0) {
                    ...
                    $app->response->setBody(json_encode(['message' => 'verrai contattato quanto prima all\'email '.$email]));
                    //FIXME DA FARE
                } else {

                    //19990324 - Ricerca ASA E/G
                    $egAsa = R::findOne('asa_anagrafica_eg', ' codicesocio = ? and datanascita = ?', [$codicecensimento, $datanascita]);
                    if ($egAsa == null) {
...
//                    }
                    ...
                    $drm_registration_prev = R::findOne('registration', ' codicecensimento = ? and type = ?', [$egAsa->codicesocio, 'EG']);
                    ...
                    $to = [$email => $egAsa->nome.' '.strtoupper($egAsa->cognome[0]).'.'];
        ...
        $app->post('/step2/:token', function ($token) use ($app) {
            ...
            try {
                $findToken = R::findOne('registration', ' token = ? and completato = ?', [$token, 0]);
                if (null == $findToken) {
                    throw new Exception('Token '.$token.' non valido', Errori::PORTAL_INVALID_TOKEN_STEP);
                }
                ...
                if (count($capoRepartoAttualeArray) > 0) {
                    ...
                    $findTokenRegistrationCC = R::findOne('registration', ' email = ? and type = ?', [$emailCapoReparto, 'CC']);
                    ...
                    $to = [$emailCapoReparto => $nomeCapoReparto.' '.strtoupper($cognomeCapoReparto[0]).'.'];
